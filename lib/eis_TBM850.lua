...
function init_eis_TBM850_lib(instrument_operator)
	...
	local function TBM850_new_trq(torque, green_lo_TRQ, green_hi_TRQ, yellow_lo_TRQ, yellow_hi_TRQ, red_lo_TRQ, red_hi_TRQ)
	...
	local function TBM850_new_prop_rpm(rpm)
		if eis_visible then 
			img_rotate(img_prop_rpm_pointer, (rpm[1]/2200) * 160 - 100)
			txt_set(txt_prop_rpm, var_round(rpm[1],0) )
		end
	end
	xpl_dataref_subscribe("sim/cockpit2/engine/indicators/prop_speed_rpm", "FLOAT[8]" , TBM850_new_prop_rpm)

	local function TBM850_new_ng(gen_spd)
		if eis_visible then 
			img_rotate(img_ng_pointer, (gen_spd[1]/120) * 160 - 100)
			txt_set(txt_ng, var_round(gen_spd[1],0) )
		end
	end
	xpl_dataref_subscribe("sim/cockpit2/engine/indicators/N1_percent", "FLOAT[8]" , TBM850_new_ng)

	-- 	sim/cockpit2/engine/actuators/ignition_key	int[8]	y	enum	0 = off, 1 = left, 2 = right, 3 = both, 4 = starting
	local function TBM850_new_itt(itt, max_itt, ignition)
	...
	local function TBM850_new_oil_press(psi)
		if eis_visible then 
			move(img_oil_psi_pointer, ((psi[1]/150) * 130 + 3), nil, nil, nil)
			txt_set(txt_oil_psi, var_round(psi[1],0) )
		end
	end
	xpl_dataref_subscribe("sim/cockpit2/engine/indicators/oil_pressure_psi", "FLOAT[8]" ,TBM850_new_oil_press)

	local function TBM850_new_oil_temp(celsius)
		if eis_visible then 
			move(img_oil_temp_pointer, ((celsius[1]/150) * 130 + 3), nil, nil, nil)
			txt_set(txt_oil_temp, var_round(celsius[1],0) )
		end
	end
	xpl_dataref_subscribe("sim/cockpit2/engine/indicators/oil_temperature_deg_C", "FLOAT[8]"  ,TBM850_new_oil_temp)


	local function TBM850_new_cabin_press(alt, diff)
		if eis_visible then 
	...
	xpl_dataref_subscribe(	"sim/cockpit/pressure/cabin_altitude_actual_m_msl", "FLOAT" ,
							...
							TBM850_new_cabin_press)

							
	local function TBM850_new_fuel(fuel, flow)
		if eis_visible then 
	...
	local function TBM850_new_electrics(battA, genA, battV, essV)
		if eis_visible then 
	...
	xpl_dataref_subscribe(	"sim/cockpit2/electrical/battery_amps", "FLOAT[8]",
							...
							TBM850_new_electrics)

	local function TBM850_new_elevator_trim(trim)
		if eis_visible then 
			move(img_elev_pointer, nil, (trim * 55 + 600), nil, nil) -- min 545 - avg 600 - max 655, -1 = down, 1 = up
		end
	end
	xpl_dataref_subscribe("sim/cockpit2/controls/elevator_trim", "FLOAT"  ,TBM850_new_elevator_trim)

	local function TBM850_new_flaps(flaps)				--	0 = up; 0.2 = T/O; 1= LND
		if eis_visible then 
			if flaps == 0 then
	...
	xpl_dataref_subscribe("sim/flightmodel2/controls/flap1_deploy_ratio", "FLOAT" ,TBM850_new_flaps)
