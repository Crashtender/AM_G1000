...
function init_eis_lib(instrument_operator)
	...
	local function new_oil_press(psi, red_lo_oilP, red_hi_oilP)
	...
	local function new_egt(itt, max_itt, ignition)
	...
	local function new_vac(value, red_lo, red_hi)
	...
	xpl_dataref_subscribe(	"sim/cockpit/misc/vacuum", "FLOAT",
							...
							"sim/aircraft/limits/red_hi_vac", "FLOAT", new_vac)
	
	
	
	local function new_oil_temp(celsius, max_temp)
	...
	xpl_dataref_subscribe(	"sim/cockpit2/engine/indicators/oil_temperature_deg_C", "FLOAT[8]",
							"sim/aircraft/engine/acf_max_OILT", "FLOAT", new_oil_temp)

		
	local function new_fuel(flow, quan, max)
